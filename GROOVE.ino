...
int atsuryoku_array[10] = {0};
int mage1_array[10] = {0};//小指
int mage2_array[10] = {0};//薬指
int mage3_array[10] = {0};//中指
int mage4_array[10] = {0};//人差し指
int mage5_array[10] = {0};//親指
int sensor_array_index = 0;
...
void led_timer() {
  ...
  if (rsvData == 1) {
    ...
    float beat = 100 * 60 / (125.0); // 150bpm
    if (timer_count >= beat) {
      timer_count = 0;
    }
    if (sensor_interrupt_flag == false) {
      int brightness = abs(int(map(timer_count, 0, int(beat) - 1, -255, 255)));
    ...
    float beat = 100 * 60 / 108.0; // 108bpm
    if (timer_count >= beat) {
      timer_count = 0;
    }
    if (sensor_interrupt_flag == false) {
...
void loop() {
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
    ...
    if(sensor_array_index >= SENSOR_ARRAY_NUM){
      sensor_array_index = 0;
    }
    atsuryoku_array[sensor_array_index] = analogRead(A0);
    mage1_array[sensor_array_index] = analogRead(A1);//小指
    mage2_array[sensor_array_index] = analogRead(A2);//薬指
    mage3_array[sensor_array_index] = analogRead(A3);//中指
    mage4_array[sensor_array_index] = analogRead(A6);//人差し指
    mage5_array[sensor_array_index] = analogRead(A7);//親指
    ...
    for(int i=0; i<SENSOR_ARRAY_NUM; i++ ){
      atsuryoku += atsuryoku_array[i];
      mage1 += mage1_array[i];//小指
      mage2 += mage2_array[i];//薬指
      mage3 += mage3_array[i];//中指
      mage4 += mage4_array[i];//人差し指
      mage5 += mage5_array[i];//親指
    }
    
//    atsuryoku /= SENSOR_ARRAY_NUM;
    atsuryoku = atsuryoku_array[sensor_array_index];
    mage1 /= SENSOR_ARRAY_NUM;//小指
    mage2 /= SENSOR_ARRAY_NUM;//薬指
    mage3 /= SENSOR_ARRAY_NUM;//中指
    mage4 /= SENSOR_ARRAY_NUM;//人差し指
    mage5 /= SENSOR_ARRAY_NUM;//親指
    ...
    // point
    if (!hitosashiyubi && nakayubi && kusuriyubi) {
...
//    else if (oyayubi && hitosashiyubi && nakayubi && kusuriyubi) {
    else if (hitosashiyubi && nakayubi && kusuriyubi) {
